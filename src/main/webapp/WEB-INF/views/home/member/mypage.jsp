...
$(document).ready(function(){
	$("#btn_member_disabled").on("click",function(){
		if(confirm("정말로 탈퇴 하시겠습니까? 로그인이 불가능합니다")){
				...
				<fieldset>
					...
					<ul class="app_list">
						...
						<li class="clear">
							...
							<div class="app_content"><input type="password" name="user_pw_chk" class="w100p" id="password_chk_lbl" placeholder="비밀번호를 다시 입력해주세요" /></div>
						</li>
						<li class="clear">
						...
						<li class="clear">
							...
							<div class="app_content radio_area">
								<select class="gender" disabled>
									...
									<option value="ROLE_USER"<c:out value="${(memberVO.levels eq 'ROLE_ADMIN')?'selected':''}" />>관리자</option>
								</select>
								<input type="hidden" name="levels" value="${memberVO.levels}" readonly>
							</div>
						</li>
						<li class="clear">
							<label for="gender_lbl" class="tit_lbl pilsoo_item">탈퇴여부</label>
							<div class="app_content radio_area">
								<input <c:out value="${(memberVO.enabled eq 'true')?'checked':''}" /> disabled type="radio" class="css-radio" id="enabled_lbl" />
								<label for="enabled_lbl">회원사용</label>
								<input <c:out value="${(memberVO.enabled eq 'false')?'checked':''}" /> disabled type="radio" class="css-radio" id="disabled_lbl" />
