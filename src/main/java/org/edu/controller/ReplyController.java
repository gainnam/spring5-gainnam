...
public class ReplyController {
	...
	public ResponseEntity<Map<String, Object>> reply_list(@PathVariable("page") Integer page, @PathVariable("bno") Integer bno) throws Exception {
		...
		pageVO.setTotalCount(replyCount);//젼체 댓글 갯수 구해서 set하는 순간이 필수 prev, next 구할 때 필요
		//페이징 계산식 처리 끝
		//현제 게시물에 달린 댓글 전체갯수 구하기: 게시물 관리 테이블에 있는 reply_count 를 가져다가 사용
		...
		try {
			...
			if(replyList.isEmpty()) {
				...
				result = new ResponseEntity<Map<String,Object>>(HttpStatus.NO_CONTENT);//code 204
			}else {
				resultMap.put("replyList", replyList);
				//debugging/System.out.println("댓글 개수 확인: " + replyList.size());
				resultMap.put("pageVO", pageVO); //페이징처리 때문에 추가
				// resultMap를 Json데이터로 반환하려면, jackson-databind 모듈이 필수(pom.xml)
				result = new ResponseEntity<Map<String, Object>>(resultMap, HttpStatus.OK);//code200
			}
		} catch (Exception e) {
			result = new ResponseEntity<Map<String,Object>>(HttpStatus.INTERNAL_SERVER_ERROR);//code500
		}
		return result;
	}
	...
	public ResponseEntity<String> reply_delete(@PathVariable("bno") Integer bno, @PathVariable("rno") Integer rno) {
		...
		try {
			...
			replyDAO.updateCountReply(bno);//서브쿼리를 사용해서 tbl_board테이블의 reply_count필드를 업데이트
		} catch (Exception e) {
			result = new ResponseEntity<String>(HttpStatus.BAD_REQUEST);
		}
		return result;
	}
	//댓글 수정 메서드(아래) 전송방식 POST(전통방식)가 아니고, PATCH(최근방식 RestAPI)를 사용
	...
	public ResponseEntity<String> reply_write(@RequestBody ReplyVO replyVO) {
		//@RequestBody class는 ajax로 보내온 폼데이터를 ReplyVO 클래스에 바인딩 시켜주는 애노테이션 클래스
		...
		return result;//ResponseEntity 객체클래스형 String값을 ajax로 호출한 페이지에 반환
	}
